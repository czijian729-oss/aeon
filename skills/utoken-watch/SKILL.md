---
name: utoken-watch
description: "Watch µToken (uToken) launchpad on Robinhood chain for open-for-launch signals: dev wallet activity, factory token deployment, official tweets, and site availability. Report any change immediately. read-only."
metadata:
  title: uToken Watch
  mode: read-only
  category: crypto
...
The operator is waiting for **µToken (uToken)** — a **Uniswap v4 launchpad** building on **Robinhood chain** — to open for token deployment. The dev wallet has already deployed the full V4 launchpad contract suite (factory, hooks, liquidity helper, swapper, marketplace, offers), but the launchpad has **NOT opened for token deployment yet**. Your job each run: scan for launch signals and report **changes** vs the last run. Signal, not noise.

## Key facts

- **Dev wallet (deployer / operator):** `0x02ed43292c6be3f49f2b287c499c77560e426e83` — the operator of both upeg (a previous project) and uToken. Deployed the entire V4 launchpad suite on 2026-08-18 ~15:56 UTC. Watch its new transactions & new contract deployments.
- **Factory (token launcher):** `0x83C33B39b086B03661a510Ec70a91D4d1800a3Fe` — verified. A **new `created_contract` from this factory = the launchpad has deployed its first token / opened for deployment** 🔥. Other wallets (0x87B53fdcD3, 0x8523d26248) are already calling it — watch closely.
...
> ⚠️ Do NOT watch `0x44b28991b167582f18ba0259e0173176ca125505` — that is the operator's PREVIOUS project (upeg), not uToken.
...
- `https://robinhoodchain.blockscout.com/api/v2/addresses/0x02ed43292c6be3f49f2b287c499c77560e426e83/transactions`
  ...
  - Any item with `created_contract` not null = the dev deployed a **new contract** (could be launch infra or a token). Report with the contract address + explorer link.

### 3. Scan factory for token deployment (the big one)

- `https://robinhoodchain.blockscout.com/api/v2/addresses/0x83C33B39b086B03661a510Ec70a91D4d1800a3Fe/transactions`
  - Look for any tx where `created_contract` is not null. That is the **launchpad deploying a token** — OPEN FOR LAUNCH 🔥. Report immediately with the token address + explorer link.
...
- If **no change**: output a one-line "no change" status (include the current snapshot: dev latest tx? factory deployed a token? site code? latest tweet date). Do NOT notify.

Include in every report:
- Dev wallet: latest tx time/hash + any new deployments
- Factory: deployed a token? (created_contract) + latest activity
